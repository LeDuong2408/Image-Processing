...
def ConnectedComponent(imgin):
    ...
    dem, label = cv2.connectedComponents(temp_gray)
...
def CountRice(imgin):
    ...
    ret, temp_gray = cv2.threshold(temp_gray, 100, L-1, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
    temp_gray = cv2.medianBlur(temp_gray, 3)
    dem, label = cv2.connectedComponents(temp_gray)
